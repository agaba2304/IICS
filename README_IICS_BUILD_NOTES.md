...
| Loop over a list | `list:head()`/`list:tail()` XQuery functions (namespace `xmlns:list="urn:activevos:spi:list:functions"`, already declared on every `<process>` element in this build) inside a Loop/While step | **CORRECTED — see "Third pass" below.** `runForEach="true"` on a `<subflow>` is *not* a general loop mechanism: per Informatica's own docs/KB it only drives a Subprocess step whose called process's "Applies To" is a queryable Process Object (JDBC/Salesforce-style connection object with a Run-on-list-with-WHERE-condition setting), and it cannot bind a scalar input to a child of an arbitrary in-memory list field. For a plain XQuery-built list (which is what every loop in this build actually is), the documented idiom is `let current := list:head(list); let list := list:tail(list)` repeated inside a native Loop step while the list is non-empty. `p_ProcessOneEmployee` and `p_dayforce_cpd_integration` now pop one item at a time into a scalar `tmp_Current*` field this way before each subflow call; the subflow itself is `runForEach="false"`. This is why a new sub-process, `p_ProcessOneEmployee`, was introduced to hold the whole per-employee body — it's called once per pop of the Orchestrator's list |
...
- **Lower confidence, flagged inline in the affected files**:
  - The native Loop/While step's own XML element/attributes — no real export containing a populated one was found, so `p_ProcessOneEmployee` and `p_dayforce_cpd_integration` currently only contain the `list:head()`/`list:tail()` pop-one-item assignment plus a single (non-looped) subflow call; **the actual loop wrapper still has to be added by hand in Process Designer's palette** (drag a Loop step, type "While", condition e.g. `count($temp.tmp_PhonesToAdd/*) > 0`, around each `asn-pophead-*`/subflow pair). Everything else about the per-item mechanism (the `list:` extension functions, popping into a scalar `tmp_Current*` field, `runForEach="false"` on the wrapped subflow) is confirmed against Informatica's own documentation and KB articles (see "Third pass" below).
...
**Third pass (current):** the `runForEach="true"` guess from the second pass turned out to be wrong — Process Designer's live validator rejected `temp.tmp_PhonesToAdd/phoneTypeCode` etc. as a subflow input source with "the source field ... is undefined" on every one of the Add/Update/Delete-phone steps. `docs.informatica.com`/`knowledge.informatica.com` still block automated fetches directly, but their content is indexed and summarized by web search, which surfaced the real mechanism: `runForEach` on a Subprocess step is documented as an "Iteration Rule" that only applies when the called process's "Applies To" is a queryable Process Object (JDBC/Salesforce-style), with a "Run on each object in list" + WHERE-condition setting — not a way to loop an arbitrary XQuery-built list. The documented mechanism for that (Informatica KB "Create a loop to iterate over any list in CAI", "Head and Tail Functions: How to Iterate Over a List of Objects") is a native Loop step using the `list:head()`/`list:tail()` extension functions — which explains why `xmlns:list="urn:activevos:spi:list:functions"` was already declared, unused, on every process file in this build from the very first pass. All `runForEach="true"` subflow calls in this project (the three phone-reconciliation steps and the Orchestrator's employee loop) have been converted to the pop-one-item-via-`list:head()`/`list:tail()` pattern; see the corrected table row above and each file's own notes for what's still a manual step (wrapping the pop+call pair in an actual Loop step, since no real export with a populated one was available to copy).
...
2. Pay special attention to wrapping each `asn-pophead-*`/subflow pair (in `p_ProcessOneEmployee` and `p_dayforce_cpd_integration`) in an actual Loop/While step via Process Designer's palette, and to the `p_ProcessOneEmployee` fault container — these are the two areas flagged above as lower-confidence.
...
| 6g | Real, distinct bug from 6f (this one is a field *type* mismatch, not a Service-vs-assignment resolution difference): `tmp_DayforcePhones`/`tmp_PhonesToAdd`/`tmp_PhonesToUpdate`/`tmp_PhonesToDelete` were declared `type="string"` with `multiSelect="true"`, but `dp-comparephones`'s XQuery actually populates them with `<phone>` XML elements (matching how `tmp_Contacts`/`tmp_ExistingPhones` are correctly declared `type="xml"` right next to them). A plain `string` field has no sub-fields, so Process Designer correctly rejected `temp.tmp_PhonesToAdd/phoneTypeCode` etc. as source fields on the `sf-addphones`/`sf-updatephones`/`sf-deletephones` `runForEach` subflow calls: `"The source field tmp_PhonesToAdd/phoneTypeCode for the input parameter in_phoneTypeCode is undefined."` Fixed by changing all 4 fields to `type="xml"` (dropping `multiSelect`, matching `tmp_Contacts`/`tmp_ExistingPhones`'s style — an `xml`-typed field can hold a sequence of elements without needing `multiSelect`). **Confirmed clean of other instances of this bug** — grepped every process for `multiSelect` combined with `type="string"` and this was the only occurrence. Only 6 of the reported 19 validation errors are explained by this fix (the `phoneTypeCode`/`diallingCode`/`number` triplet × add/update, plus `phoneTypeCode` alone × delete); the remaining ~13 weren't visible in the screenshot provided — waiting on the full list to see if they're the same class of issue or something new. | `p_ProcessOneEmployee` |
