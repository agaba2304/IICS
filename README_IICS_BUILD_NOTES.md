...
| 6d | **Genuine new production gap, not just test data**: removing `hrInformation.lineManager` entirely (after its earlier fake VINCI ID failed a Luhn check, item 6b) turned out to be wrong — a real 422 (`"'Associated Line Manager Person Id' must not be empty."`) confirms CPD **requires** a line manager reference on this contract shape, it just needs a *valid* one. There is no Dayforce-to-manager-VinciId mapping in this build at all — build guide 4.8/4.9 never surfaced one. For now, re-added `lineManager` using a real, confirmed-valid VINCI ID (`04470043`, the "Yann Chaitas" test person pulled from a real `Get Persons` response earlier in this project) purely so testing can proceed — **this is not a real manager mapping and must not go to production as-is.** Before go-live, this needs either a genuine Dayforce field that identifies an employee's manager (translated to that manager's CPD VINCI ID, likely via a `p_SearchPerson`-style lookup) or confirmation from the client that this field can be omitted/defaulted for certain employee categories. | `sc-api-cpd` `CreatePerson` action, eventually `p_CreatePersonAndContract` |
