...
### A REST action's Body field is parsed as XQuery, not literal text with `{field}` substitution

Discovered while debugging `CreatePerson`'s JSON body in Process Designer's own "Expression Editor for Body" (Type: `xQuery`), through a sequence of real errors:

1. The original hand-authored body — a literal JSON object starting with a bare `{` — silently lost its outer braces when the editor displayed it (not a hard error, just corrupted content).
2. Guessing that literal `{`/`}` needed **escaping by doubling** (`{{`/`}}`) produced a hard parse error: `XPST0003: Unexpected token "{" at start of expression`, then (after doubling) `XPST0003: expected "}", found ":"`. Doubling is not a valid escape here — it's read as two separate, nested enclosed-expressions.
3. The actual fix, confirmed working: keep the outer object wrapper as a **single** `{`/`}` (i.e. exactly what plain JSON looks like), and reference an action's own input parameters *inside quoted string values* as `"{ $ParamName }"` — a single-brace enclosed expression containing a genuine XQuery variable reference (`$ParamName`, confirmed via Process Designer's own "Fields → Input Parameters" panel, which inserts that exact syntax when you click a field). Plain `{ParamName}` (no `$`) is **not** evaluated — it stays literal text.

So the working pattern for a JSON body is: the enclosing `{ ... }` is literal JSON, and every dynamic value is `"{ $ParamName }"` (or a bare, unquoted `{ $ParamName }` for a non-string JSON value). This is different from the `{ParamName}` (no `$`) convention used everywhere else in these connectors — in URLs and headers, where it's plain string substitution rather than an XQuery-typed field. Applied this fix to every JSON-bodied action in `sc-api-cpd`: `CreatePerson` and `CreateOrUpdateContract` (confirmed working directly), plus `UpdatePerson`, `AddPhoneNumber`, and `UpdatePhoneNumber` (same fix applied proactively since they had the identical `{ParamName}`-without-`$` bug, but **not yet individually tested** — check these three the same way if they error).

**Note on syncing with live edits**: the user tested some of these fixes directly in Process Designer's UI in parallel (adding `nogen`/`testWith` attributes via the "Test" panel, and a scratch `test` action pasting in a raw curl example — that action's `url` is literally the curl command text, not a real URL, so it will never work if invoked; it was left in place untouched rather than deleted, since it doesn't interfere with anything). That live-edited export was used as the base for this round's fixes (rather than this repo's previously-separately-evolved copy) specifically to preserve those `testWith` values and catch a regression: `CreatePerson`'s body had reverted to the invalid doubled `{{`/`}}` braces, and `SearchPersonByExtId`'s `extSystemId` had been changed to `1` (probably copied from the generic doc example, or from testing against the sample person's own system-1 identifier) instead of `11`, the value the design doc and `CreatePerson`'s own payload consistently use for Dayforce. Both were reverted to the confirmed-correct values. Also added `testWith` sample values (from the documented Create Person example) to the 5 new contract-related inputs on `CreatePerson`, matching the style already used on its other parameters.
...
| 6b | Several CPD Create Person/Contract schema fields have no confirmed Dayforce mapping and were deliberately left out rather than guessed: `personVinciId` (CPD-generated), `titleCode`, `contracts[].organizationalUnitAssignment.service`, `contracts[].hrInformation` (payroll job title, managers, professional phone numbers, etc.), `contracts[].extraInformation` beyond `costCenterCode`, and top-level `auditTrail`. Add these if a later CPD validation error demands them. | `sc-api-cpd` `CreatePerson`/`CreateOrUpdateContract` actions |
