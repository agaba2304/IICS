...
|---|------|-------------------|
| 1 | ~~CPD hostname is a placeholder (`{cpd-host}`)~~ — **resolved**: confirmed the real test host is `api-tst.vinci-construction.net` (same one the original "Get Persons"/"Get purchase orders" dummy actions already used), and while fixing this found the 7 newly-built actions were also missing the `/hr` path prefix entirely (`{cpd-host}/v1/persons` instead of `api-tst.vinci-construction.net/hr/v1/persons`) — an actual bug, confirmed by the user hitting a 400 ("Both external identifiers and extSystem must be provided or none of them") on `SearchPersonByExtId` that a working example call resolved. Fixed all 7 URLs (`SearchPersonByExtId`, `CreatePerson`, `UpdatePerson`, `CreateOrUpdateContract`, `AddPhoneNumber`, `UpdatePhoneNumber`, `DeletePhoneNumber`) to `https://api-tst.vinci-construction.net/hr/v1/persons...`, inferring the `/hr` prefix applies uniformly to every `persons`-rooted endpoint (`/persons/{id}/contracts/...`, `/persons/{id}/contracts/{id}/phoneNumber/...`) since they're all sub-resources of the same confirmed-correct base path. **Only directly confirmed for `SearchPersonByExtId`** — test each of the other 6 and report back if any needs a different path. | `sc-api-cpd` — every CPD action |
